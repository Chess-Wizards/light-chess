...
class PositionsUnderThreat(object):
    ...
    def is_position_enemy_or_empty(pos: Position, colour: Colour, board: Board) -> bool:
        ...
        if Board.is_position_on_board(pos, board):
            return PositionsUnderThreat.is_position_enemy(pos, colour, board) or board.is_position_empty(pos)
        else:
            return False

    @staticmethod
    def check_positions(pos: Position, colour: Colour, board: Board, shifts: list) -> List[Position]:
        ...
        for shift_x, shift_y in shifts:
            ...
            if PositionsUnderThreat.is_position_enemy_or_empty(Position(pos.x+shift_x,
                                                                         pos.y+shift_y),
                                                                colour, board):
                # Add shift
    ...
    @staticmethod
    def check_positions_with_obstacles(pos: Position, colour: Colour, board: Board, shifts: list) -> List[Position]:
        """Take input list of shifts and verify which of them are possible.
        ...
        for shift_x, shift_y in shifts:
            # Check if shift is possible, otherwise break the loop.
            if not PositionsUnderThreat.is_position_enemy_or_empty(Position(pos.x+shift_x,
                                                                             pos.y+shift_y),
                                                                    colour, board):
                break
            # Add shift
            ...
            if PositionsUnderThreat.is_position_enemy(Position(pos.x+shift_x,
                                                                pos.y+shift_y),
                                                       colour, board):
                break
        return positions_under_threat
    ...
    @staticmethod
    def all_positions_under_threat_for_side(colour: Colour, board: Board) -> List[Position]:
        positions_under_threat = []
        for pos in board.get_positions_for_side(Colour.change_colour(colour)):
            positions_under_threat.extend(PositionsUnderThreat.positions_under_threat(pos, board.get_piece(pos), board))
        return positions_under_threat
    ...
    @staticmethod
    def positions_under_king_threat(pos: Position, colour: Colour, board: Board) -> List[Position]:
    ...
    @staticmethod
    def positions_under_queen_threat(position: Position, colour: Colour, board: Board) -> List[Position]:
        """Return list of positions under threat by queen.
           Can be calculated by combining returns from bishop and rook methods.
        """

        return [*PositionsUnderThreat.positions_under_rook_threat(position, colour, board),
                *PositionsUnderThreat.positions_under_bishop_threat(position, colour, board)]

    @staticmethod
    def positions_under_bishop_threat(pos: Position, colour: Colour, board: Board) -> List[Position]:
        ...
        for shifts in [shifts_up_right, shifts_down_right, shifts_down_left, shifts_up_left]:
            positions_under_threat = [*positions_under_threat,
                                      *PositionsUnderThreat.check_positions_with_obstacles(pos, colour, board, shifts)]
        return positions_under_threat

    @staticmethod
    def positions_under_knight_threat(pos: Position, colour: Colour, board: Board) -> List[Position]:
    ...
    @staticmethod
    def positions_under_rook_threat(pos: Position, colour: Colour, board: Board) -> List[Position]:
    ...
    @staticmethod
    def positions_under_pawn_threat(pos: Position, colour: Colour, board: Board) -> List[Position]:
